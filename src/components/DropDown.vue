        ...
        <ul dir="rtl">
            <template v-if="items.listWithKeys">
                <div v-for="item in items.listWithKeys">
                    <li><button class="outline" :key="item.key" @click="toggleDropdown" :popovertarget="item.key">{{
                            item.value }}</button></li>
                </div>
            </template>
            <template v-else>
                <div v-for="(item, index) in items.list">
                    <li><button class="outline" :key="index" @click="toggleDropdown" :popovertarget="item">{{ item
                            }}</button></li>
                </div>
            </template>
        </ul>
